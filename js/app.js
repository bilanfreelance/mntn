...
let bodyLinks = document.querySelectorAll('.landing__link');

if (bodyLinks.length > 0) {
	for (let i = 0; i < bodyLinks.length; i++) {
		const bodyLink = bodyLinks[i];
		bodyLink.addEventListener("click", function (e) {
			for (let i = 0; i < bodyLinks.length; i++) {
				const bodyLink = bodyLinks[i];
				bodyLink.classList.remove('_active');
			}
			bodyLink.classList.add('_active');
